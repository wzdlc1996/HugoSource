...
<div>$$\ket{\bm{z}}=\exp(\bm{z}\cdot \hat {\bm{a}}^\dagger)\ket{0}=\exp(\sum_{i}z_i \hat a_i^\dagger)\ket{0}$$</div>

<em>[Theorem]</em> : The overlap between two coherent states is:

<div>$$\bra{\bm{z}}\bm{z}'\rangle=\exp(\bm{z}^*\cdot \bm{z}')$$</div>

{{%fold "Proof"%}}

With that `$\hat b(\bm{z})^\dagger=\bm{z}\cdot \hat {\bm{a}}^\dagger$` , we have:

<div>$$[\hat b(\bm{z}), \hat b(\bm{z}')^\dagger]=\bm{z}^* \cdot \bm{z}'$$</div>
...
<div>$$\td \mu(\bm{z})= \prod_i \frac {\td (\text{Re}z_i)\td (\text{Im}z_i)} {\pi}\exp\Big(-\sum_i |z_i|^2\Big)$$</div>

one has:

<div>$$\int \td \mu(\bm{z})\ket{\bm{z}}\bra{\bm{z}}=1$$</div>

{{%fold "Proof"%}}

We need only prove that `$\int \td \mu(\bm{z})\ket{\bm{z}}\bra{\bm{z}}\psi\rangle =\ket{\psi}$` . hold for arbitrary `$\ket{\psi}$` . For the fact that:

<div>$$\hat a_i^\dagger \ket{0}=\frac {\partial} {\partial z_i} \ket{\bm{z}} \Bigg|_{\bm{z}=0}$$</div>
...
<div>$$\text{Tr}\hat A:=\sum_{n_1,n_2,\cdots} \bra{n_1,n_2,\cdots}\hat A\ket{n_1,n_2,\cdots}=\int \td \mu(\bm{z}) \bra{\bm{z}}\hat A\ket{\bm{z}}$$</div>

{{%fold "Proof"%}}

With the expansion of coherent state:

<div>$$\ket{\bm{z}}=\sum_{n=0}^{\infty} \frac 1 {n!} \Big(\sum_i z_i \hat a_i^\dagger\Big)^n \ket{0}=\sum_{n=0}^{\infty}\sum_{k_1+\cdots+k_m=n} \frac {z_1^{k_1}\cdots z_m^{k_m}} {k_1!\cdots k_m!}\prod_{j=1}^m \hat a_j^{\dagger k_j}\ket{0}$$</div>
...
<div>$$\int \td \mu(\bm{z}) \ \prod_{i=1}^m z_i^{k_i} z_i^{* l_i}=0$$</div>
...
<div>$$\int \td \mu(\bm{z}) \prod_{i=1}^m z_i^{k_i}z_i^{* l_i}=\delta_{k,l}\prod_{i=1}^m k_i!$$</div>
...
<div>$$\ket{n_1,\cdots}:=\frac 1 {\sqrt{\prod_k n_k!}} \prod_k\hat a_k^{\dagger n_k}\ket{0}=\frac 1 {\sqrt{\prod_k n_k!}}\prod_k \frac {\partial^{n_k}} {\partial z_k^{n_k}}\ket{\bm{z}}\Bigg|_{\bm{z}=\bm{0}}$$</div>
...
<div>$$[\hat a_i , e^{\bm{z}\cdot \hat{\bm{a}}^\dagger}]=\sum_{n=1}^{\infty} \frac 1 {n!} [\hat a_i,(\sum_j z_j \hat a_j^\dagger)^n]=\sum_{n=1}^{\infty}\frac n {n!} z_i^n (\bm{z}\cdot \hat {\bm{a}}^\dagger)^{n-1}=z_i e^{\bm{z}\cdot \hat {\bm{a}}^\dagger}$$</div>
...
<div>$$\frac {\bra{\bm{z}}\hat N\ket{\bm{z}}} {\bra{\bm{z}}\bm{z}\rangle} = \sum_i |z_i|^2$$</div>
...
<div>$$\frac {\bra{\bm{z}}(\hat N-\bar N)^2\ket{\bm{z}}} {\bra{\bm{z}}\bm{z}\rangle} =\frac {\bra{\bm{z}}\hat N^2 \ket{\bm{z}}} {\bra{\bm{z}}\bm{z}\rangle } - \bar N^2 =\sum_i |z_i|^2 = \bar N$$</div>
...
<div>$$\ket{\psi}=\int \td \mu(\bm{z}) \ket{\bm{z}}\bra{\bm{z}}\psi\rangle = \int \td \mu(\bm{z}) \ \psi(\bm{z}^*)\ket{\bm{z}}$$</div>
...
<div>$$\bra{\bm{z}}A(\hat {\bm{a}}^\dagger,\hat {\bm{a}})\ket{\bm{z}'}=A(\bm{z}^*,\bm{z}') \exp(\bm{z}^*\cdot \bm{z}')$$</div>
...
<div>$$\ti\hbar \frac {\partial \psi(\bm{z}^*,t)} {\partial t}=\int \td \mu(\bm{z}') H(\bm{z}^*,\bm{z}',t)e^{\bm{z}^*\cdot \bm{z'}}\psi(\bm{z}'^*,t)$$</div>

{{%fold "Proof"%}}

The generic form of Schrodinger equation is:

<div>$$\ti\hbar \frac {\td} {\td t}\ket{\psi(t)}=\hat H \ket{\psi(t)}$$</div>

Here we let:

<div>$$\ket{\psi(t)}=\int \td \mu(\bm{z})\ket{\bm{z}}\psi(\bm{z}^*,t)$$</div>
...
<div>$$\big\{1,\ \xi_\alpha,\ \xi_{\alpha_1}\xi_{\alpha_2},\ \cdots,\ \xi_{\alpha_1}\cdots\xi_{\alpha_n}\big\}$$</div>

A `Grassmann number` can be written as:

<div>$$g=z_0 + \sum_\alpha z_\alpha \xi_\alpha+\sum_{\alpha_1,\alpha_2}z_{\alpha_1,\alpha_2}\xi_{\alpha_1}\xi_{\alpha_2}+ \cdots$$</div>
...
<div>$$f(g=a_0+\sum_{l=1}^N \sum_{i_1\lt \cdots\lt i_l}a_{i_1,\cdots,i_l}\xi_{i_1}\cdots \xi_{i_l} )=A_0(a)+\sum_{l=1}^N \sum_{i_1\lt \cdots\lt i_l}A_{i_1,\cdots,i_l}(a)\xi_{i_l}\cdots\xi_{i_l}$$</div>
...
<div>$$\int \td \xi' \delta(\xi,\xi')f(\xi')=f(\xi)$$</div>
...
<div>$$(f,g):=\int \td\xi^* \td \xi e^{-\xi^*\xi}f^*(\xi)g(\xi^*)=f_0^* g_0+f_1^*g_1$$</div>

{{%fold "Proof"%}}

With that `$f(\xi)=f_0+f_1\xi \ ; \ g(\xi)=g_0+g_1\xi$`, And:

<div>$$e^{-\xi^*\xi}=1-\xi^*\xi+\frac {\xi^*\xi\xi^*\xi} {2!}+\cdots=1-\xi^*\xi$$</div>
...
<div>$$\int \td \eta f(\eta) = (\det M)^{-1} \int \td \xi f(M\xi+b)$$</div>
...
<div>$$\int \td \xi \ f(\xi+b)=\int \td \xi \ f(\xi)$$</div>

With the mathematical induction, when `$N=1$` , it is:

<div>$$\int \td \xi \ f(\xi+b)=\int \td \xi (u+v\xi+vb)=v=\int \td f(\xi)$$</div>
...
<div>$$f(\eta)=a+\eta_1b+\cdots + \eta_1\cdots\eta_N z$$</div>
...
<div>$$\int \td \ \xi f(M\xi+b)=\det M \cdot \int \td \eta f(\eta)$$</div>
...
<div>$$\int \prod_{i} \td \eta_i^*\td \eta_i \ \exp\Big(-\eta^\dagger H\eta+J^\dagger \eta +\eta^\dagger J\Big)=(\det H) e^{-J^\dagger H^{-1}J}$$</div>
...
<div>$$\bra{\xi}\xi'\rangle = e^{\xi^*\cdot \xi'}$$</div>

{{%fold "Proof"%}}

With the definition of Fermion coherent state and the orthogonality of Fermion product state, if:

<div>$$\ket{\xi}=\ket{0}+\sum_i c_i(\xi) \hat a_i^\dagger \ket{0} + \sum_{i,j} c_{ij}(\xi) \hat a_i^\dagger \hat a_j^\dagger \ket{0}+\cdots$$</div>

then:

<div>$$\bra{\xi}\xi'\rangle = 1-\sum_i c_i(\xi)^* c_i(\xi') + \sum_{i, j}c_{ij}(\xi)^* c_{ij}(\xi')+\cdots  $$</div>

Where the difference on sign is caused by the anti-commutation. The coefficients:

<div>$$c_i(\xi)=-\xi_i \ ; \ c_{ij}(\xi)=- \xi_i\xi_j \ ; \ \cdots \ ; \ c_{i_1,\cdots,i_n}(\xi)=(-1)^{n(n+1)/2} \xi_{i_1}\cdots \xi_{i_n}$$</div>

Used the property that

<div>$$[\xi_i \hat a_i , \xi_j\hat a_j^\dagger]=\xi_i\hat a_i \xi_j\hat a_j^\dagger - \xi_j\hat a_j^\dagger \xi_i\hat a_i=-\xi_i\xi_j\{\hat a_i,\hat a_j^\dagger\}=-\xi_i\xi_j\delta_{ij}=0$$</div>
...
<div>$$\int \prod_i \td \xi_i^* \td \xi_i e^{-\xi^*\cdot \xi}\ket{\xi}\bra{\xi}=1$$</div>

{{%fold "Proof"%}}

Because Fermion coherent states cover all `$n$`-particle states of Fermion:

<div>$$\hat a_{i_1}^\dagger \cdots \hat a_{i_n}^\dagger \ket{0}=(-1)^{n(n+1)/2} \partial_{\xi_{i_1}}\cdots \partial_{\xi_{i_n}}\ket{\xi}$$</div>
...
<div>$$\partial_{\xi_i^*} e^{-\xi^*\cdot (\xi-\xi')}=\partial_{\xi_i^*}\prod_{j}\Big(1-\xi_j^*(\xi_j-\xi'_j)\Big)=-(\xi_j-\xi'_j)=-\delta(\xi'_j,\xi_j)$$</div>

Then we have:

<div>$$\int \prod_i \td \xi_i^* \td \xi_i e^{-\xi^*\cdot \xi}\ket{\xi}\bra{\xi}\xi'\rangle=\int \prod_i \td \xi_i \delta(\xi',\xi)\ket{\xi}=\ket{\xi'}$$</div>
...
<div>$$\text{Tr} \hat A:=\sum_{n_1,\cdots}\bra{n_1,\cdots}\hat A \ket{n_1,\cdots}=\int \prod_i \td \xi_i^* \td \xi_i e^{-\xi^* \cdot \xi} \bra{-\xi}\hat A\ket{\xi}$$</div>
...
<div>$$\ket{n_1,\cdots}:=\prod_{k} \hat a_k^{\dagger n_k}\ket{0} =(-1)^{n_1+\cdots}\prod_i \partial_{\xi_i}^{n_i}\ket{0}$$</div>
...
<div>$$\ket{\psi}=\int \prod_i \td \xi_i^*\td \xi_i e^{-\xi^*\cdot \xi} \ket{\xi}\bra{\xi}\psi\rangle=\int \prod_i \td \xi_i^*\td \xi_i e^{-\xi^*\cdot \xi} \ket{\xi}\psi(\xi^*)$$</div>

The representation of operators are:

<div>$$\hat a_i \rightarrow \partial_{\xi_i^*} \ ; \ \hat a_i^\dagger \rightarrow \xi_i^*$$</div>

And the matrix elements of normal ordered operators, which means that all creation operators are at the left side of annihilation operators, is:

<div>$$\bra{\xi}A(\hat {\bm{a}}^\dagger,\hat {\bm{a}})\ket{\xi'}=e^{\xi^*\cdot \xi'}A(\xi^*,\xi')$$</div>
...
<div>$$\hat a_i \ket{\xi}=\xi_i\ket{\xi}$$</div>
...
<div>$$\ti \hbar \frac {\partial \psi(\xi^*,t)} {\partial t}=\int \prod_{i} \td \xi_i'^*\td \xi_i' e^{\xi'^*\cdot \xi'} H(\xi^*,\xi',t) \psi(\xi'^*,t)$$</div>
...
<div>$$\ket{\psi(t)}=\int \prod_i \td \xi_i^*\td \xi_i e^{-\xi^*\cdot \xi} \ket{\xi}\psi(\xi^*,t)$$</div>
