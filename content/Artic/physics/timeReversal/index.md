...
Proof: If we assume $T$ can be generated by a linear operator $\hat T$, i.e., $\forall \ket{x} \in x \in\mathscr{H}\rightarrow \hat T\ket{x} \in Tx$, then the condition means for any unitary operator $\hat U$, $\hat T^{-1} \hat U \hat T$ is also a unitary operator or an anti-unitary operator, since $T$ holds the orthonormality. 
