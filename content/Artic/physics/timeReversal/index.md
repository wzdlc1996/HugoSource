...
Proof: If we assume $T$ can be generated by a linear operator $\hat T$, i.e., $\forall \ket{x} \in x \in\mathscr{H}\rightarrow \hat T\ket{x} \in Tx$, then the condition means for any unitary operator $\hat U$, $\hat T^{-1} \hat U \hat T$ is also a unitary operator or an anti-unitary operator, since $T$ holds the orthonormality. (this proof requires the linearity or anti linearity, it is necessary)

Then, within the unitary branch, we have for any unitary operator $\hat U$, one has

$$
 \hat T^{-1} \hat U \hat T (\hat T^{-1} \hat U \hat T )^\dagger = 1 \Rightarrow \hat U \hat T \hat T^\dagger \hat U^\dagger = \hat T \hat T^\dagger
$$

Then, if we prove that the operator who commutes to any unitary operator should be identity up to a global constant factor, we have $\hat T\hat T^\dagger = 1$, thus it is unitary. A similar analysis can be done for the anti-unitary branch. 
